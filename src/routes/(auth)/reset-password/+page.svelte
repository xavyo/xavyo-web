...
<div class="space-y-6">
	<div>
		...
		<p class="mt-1 text-sm text-muted-foreground">Enter your new password below</p>
	</div>
		...
		<Alert>
			<AlertDescription>
				...
				{#if typeof $message === 'string' && $message.includes('successfully')}
					<a href={loginAfterResetUrl} class="font-medium text-primary underline-offset-4 hover:underline ml-1">Go to login</a>
				{/if}
			</AlertDescription>
		</Alert>
	{/if}
	...
	<p class="text-sm text-muted-foreground">
		<a href="/login{tenantParam}" class="font-medium text-primary underline-offset-4 hover:underline">Back to login</a>
	</p>
</div>
