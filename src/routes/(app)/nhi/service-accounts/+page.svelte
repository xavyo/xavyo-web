<script lang="ts">
	...
	async function fetchServiceAccounts() {
		...
		try {
			...
			addToast('error', 'Failed to load service accounts');
		} finally {
			isLoading = false;
		}
	}
	...
	// Fetch service accounts when filters or pagination change
	$effect(() => {
		void pagination;
...
<div class="flex items-center justify-between">
	<PageHeader title="NHI Service Accounts" description="Manage machine-to-machine credentials" />
	<div class="flex items-center gap-2">
		...
		<a
			href="/nhi/service-accounts/create"
...
{#snippet emptyState()}
	{#if hasFilters}
		<EmptyState
			title="No service accounts match your filters"
	...
	{:else}
		<EmptyState
			...
			description="Create your first machine-to-machine credential."
			icon="🔑"
		/>
	{/if}
{/snippet}
