	...
	<div class="flex gap-3">
		{#if schedule.status === 'active'}
			<form method="POST" action="?/pause" use:svelteEnhance={() => {
				...
				<button
					type="submit"
					class="inline-flex h-9 items-center rounded-md border border-yellow-500 px-4 text-sm font-medium text-warning hover:bg-yellow-50 dark:hover:bg-yellow-900/20"
				>
					Pause
				</button>
			</form>
		{:else}
			<form method="POST" action="?/resume" use:svelteEnhance={() => {
				...
				<button
					type="submit"
					class="inline-flex h-9 items-center rounded-md border border-green-500 px-4 text-sm font-medium text-success hover:bg-green-50 dark:hover:bg-green-900/20"
				>
					Resume
				</button>
			</form>
