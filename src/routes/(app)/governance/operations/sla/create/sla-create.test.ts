...
import { createSlaPolicy } from '$lib/api/governance-operations';
import { ApiError } from '$lib/api/client';
import { hasAdminRole } from '$lib/server/auth';

const mockHasAdminRole = vi.mocked(hasAdminRole);
const mockCreateSlaPolicy = vi.mocked(createSlaPolicy);
...
function makeFormData(data: Record<string, string>): Request {
	...
	return new Request('http://localhost/governance/operations/sla/create', {
		method: 'POST',
...
describe('SLA Create +page.server', () => {
	beforeEach(() => {
		vi.resetAllMocks();
	...
	describe('load', () => {
		...
		it('form has default breach_notification_enabled as true', async () => {
			const result: any = await load({
				locals: mockLocals(true)
			} as any);
		...
		it('form has default warning_threshold_percent of 75', async () => {
			const result: any = await load({
				locals: mockLocals(true)
			} as any);
	...
	describe('default action', () => {
		...
		it('calls createSlaPolicy and redirects on success', async () => {
			mockCreateSlaPolicy.mockResolvedValue({ id: 'new-sla-id' } as any);
			try {
		...
		it('returns API error message on failure', async () => {
			mockCreateSlaPolicy.mockRejectedValue(new ApiError('Duplicate name', 409));
			expect(typeof actions.default).toBe('function');
		});
	});
});

describe('SLA Create +page.svelte', () => {
	it(
