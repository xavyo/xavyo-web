	...
	const { form, errors, enhance, message } = superForm(data.form, {
		onResult({ result }) {
			if (result.type === 'redirect') {
				addToast('success', 'SCIM target updated successfully');
			}
		}
	});
...
<PageHeader title="Edit SCIM Target" description="Update SCIM target configuration" />
		...
		<form method="POST" use:enhance class="space-y-4">
			...
			<div class="space-y-2">
				...
				<select
					...
					onchange={(e) => {
						selectedAuthMethod = (e.target as HTMLSelectElement).value as 'bearer' | 'oauth2';
					}}
				>
					{#each AUTH_METHODS as method}
						<option value={method} selected={data.target.auth_method === method}>
							{authMethodLabels[method] ?? method}
						</option>
					{/each}
				</select>
				...
				<div class="space-y-4 rounded-md border border-input p-4">
					<div class="space-y-2">
						...
						<select
							...
							class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
							value={String($form.deprovisioning_strategy ?? 'deactivate')}
						>
							{#each DEPROVISIONING_STRATEGIES as strategy}
								<option value={strategy} selected={data.target.deprovisioning_strategy === strategy}>
									{deprovisioningLabels[strategy] ?? strategy}
								</option>
							{/each}
						</select>
					</div>
					...
					<div class="space-y-2">
						...
						<select
							...
							class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
						>
							<option value="on" selected={data.target.tls_verify}>Yes</option>
							<option value="off" selected={!data.target.tls_verify}>No</option>
						</select>
					</div>
					...
					<div class="space-y-2">
						...
						<Input
							...
							type="number"
							value={String($form.rate_limit_per_minute ?? '')}
						/>
					</div>
					...
					<div class="space-y-2">
						...
						<Input
							...
							type="number"
							value={String($form.request_timeout_secs ?? '')}
						/>
					</div>
			...
			<div class="flex gap-2 pt-2">
				...
				<a
					href="/settings/scim-targets/{data.target.id}"
