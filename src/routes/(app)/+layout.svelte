<script lang="ts">
	...
	const navSections: NavSection[] = $derived.by(() => {
		...
		if (data.isAdmin) {
			...
			// Applications (OIDC clients) sits next as a top-level Auth0-style entry.
			sections.unshift(
				{
					label: 'Dashboard',
					collapsible: false,
				...
				},
				{
					label: 'Applications',
					collapsible: false,
					items: [{ label: 'Applications', href: '/settings/oauth-clients', icon: KeyRound }]
				}
			);
			sections.push(
				{
				...
				},
				{
					...
					items: [
						...
						{ label: 'Risk Management', href: '/governance/risk', icon: ShieldAlert }
					]
				},
				{
