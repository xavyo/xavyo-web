...
vi.mock('$lib/api/simulations', () => ({
	updateBatchSimulationNotes: vi.fn()
}));
...
function makeEvent(body: string) {
	return {
		...
		fetch: vi.fn(),
		request: new Request('http://localhost/api/governance/simulations/batch/b1/notes', {
			method: 'PATCH',
...
describe('PATCH /api/governance/simulations/batch/:id/notes', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	it('updates notes with required fields', async () => {
		vi.mocked(updateBatchSimulationNotes).mockResolvedValue({ id: 'b1' } as any);
		const response = await PATCH(makeEvent(JSON.stringify({ notes: 'ok' })) as any);
		expect(response.status).toBe(200);
		expect(updateBatchSimulationNotes).toHaveBeenCalled();
	});

	it('does not update on invalid JSON', async () => {
		await expect(PATCH(makeEvent('{not json') as any)).rejects.toMatchObject({ status: 400 });
		expect(updateBatchSimulationNotes).not.toHaveBeenCalled();
	});
