...
<div class="space-y-4" data-testid="validation-results">
	{#if validation.valid && validation.sod_violations.length === 0}
		<div class="flex items-center gap-2 rounded-lg bg-green-50 p-4 text-success dark:bg-green-950 ">
	...
	{#if validation.sod_violations.length > 0}
		<div class="rounded-lg border border-yellow-500/50 bg-yellow-50 p-4 dark:bg-yellow-950">
			<h4 class="mb-2 flex items-center gap-2 font-medium text-warning-foreground ">
			...
			<ul class="space-y-2">
				{#each validation.sod_violations as violation}
					<li class="text-sm">
						<span class="font-medium text-warning-foreground ">{violation.rule_name}</span>
						<span class="text-warning-foreground "> — {violation.description}</span>
					</li>
				{/each}
			</ul>
			<p class="mt-2 text-xs text-warning">SoD warnings do not block submission.</p>
		</div>
	{/if}
</div>
