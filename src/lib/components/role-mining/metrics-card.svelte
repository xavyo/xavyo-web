<script lang="ts">
	...
	const trendConfig = $derived({
		up: { label: 'Up', icon: TrendingUp, class: 'text-success' },
		down: { label: 'Down', icon: TrendingDown, class: 'text-destructive' },
...
<div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 ">
	<div class="mb-3 grid grid-cols-2 gap-3">
		<div>
			<div class="text-xs text-muted-foreground ">Utilization Rate</div>
			<div class="text-lg font-semibold text-gray-900 dark:text-gray-100">{metrics.utilization_rate.toFixed(1)}%</div>
		</div>
		<div>
			<div class="text-xs text-muted-foreground ">Coverage Rate</div>
