<script lang="ts">
	...
	const statusConfig = $derived({
		...
		accepted: { label: 'Accepted', class: 'bg-success/15 text-success' },
		remediated: { label: 'Remediated', class: 'bg-info/15 text-info' },
		reviewed: { label: 'Reviewed', class: 'bg-muted text-muted-foreground ' }
	}[flag.status]);

	const deviationColor = $derived(
		flag.deviation_percent >= 75 ? 'bg-destructive' :
		flag.deviation_percent >= 50 ? 'bg-warning' :
		flag.deviation_percent >= 25 ? 'bg-warning' :
		'bg-success'
	);
